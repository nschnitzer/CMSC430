...
(define rdi 'rdi) ; arg
(define rsi 'rsi) ; dont use
(define r12 'r12) ; error handler space
(define r14 'r14) ; Need Counter for # of active handlers
  ...
  (match p
    [(Prog ds e)
     (prog (externs)
           ...
           (Mov r14 0) ;; Initialize handler counter
           (Push r12) ; save callee-saved register
           (Mov r12 rsi) ; recv handler space pointer
           ...
           (Pop r14)     ; restore callee-save register
           (Ret)
           (compile-defines ds)
...
(define (compile-e e c t?)
  (match e
    ...
    [(ExcMgr hdlrs e)   (compile-exception-manager hdlrs e c t?)]
    [(Handler p f)      (compile-handler p f c t?)]))
...
(define (compile-raise e c)
  (let ((loop-begin  (gensym 'raise))
        (loop-end    (gensym 'raise))
    ...
    (seq  (%% "entering raise")
          ...
          (Add 'r15 1)
          (Sub r12 16) ;; pop the pred and func off the error handler stack
          (Jmp loop-begin)

          (Label loop-end) ;; no error handler worked
          (Pop 'rcx)
          (Pop 'r15)
...
(define (compile-handlers hdlrs c t?)
  (match hdlrs
    [(cons x xs)  (seq (compile-e x c t?)
                       (compile-handlers xs c t?))]
    ['()  (seq)]))

(define (compile-exception-manager hdlrs e c t?)
  (seq (compile-handlers hdlrs c t?)
       (compile-e e c t?)
       (Mov 'rcx (length hdlrs))
