...
(define (sort < xs)
  (match (length xs)
    [0 '()]
    [1 xs]
    [_ (cons (first (sort-helper < xs))
          (sort < (rest (sort-helper < xs))))]))

;; Compares the first two elements of a list and swaps them if they
;; are out of order
(define (sort-helper < xs)
  (match xs
    ['() '()]
    [(list x) (list x)]
    [(cons y ys) (if (< (first ys) y)
                    (cons (first ys)
                          (sort-helper < (cons y (rest ys))))
                    (cons y (sort-helper < ys)))]))
...
(define (btn-contains? bt n)
  (match bt
    [(leaf) #f]
    [(node data left right) (if (equal? data n) #t (or (btn-contains? left n) (btn-contains? right n)))]))
