...
(define (no-begin e)
  (match e
    [(Begin e1 e2)        (Lam (gensym 'lambda) (no-begin e2) (no-begin e1))]
    [(Let x e1 e2)        (Let x (no-begin e1) (no-begin e2))]
    [(Prim1 p  e1)        (Prim1 p (no-begin e1))]
    [(Prim2 p  e1 e2)     (Prim2 p (no-begin e1) (no-begin e2))]
    [(Prim3 p  e1 e2 e3)  (Prim3 p (no-begin e1) (no-begin e2) (no-begin e3))]
    [(If    e1 e2 e3)     (If (no-begin e1) (no-begin e2) (no-begin e3))]
    [(Lam   f  xs e1)     (Lam f xs (no-begin e1))]
    [(App   e1 es)        (App (no-begin e1) (map no-begin es))]
    [(Prog  ds e1)        (Prog (map no-begin ds) (no-begin e1))]
    [(Defn  f  xs e1)     (Defn f xs (no-begin e1))]
    [x                    x]))
