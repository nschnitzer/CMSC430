...
(define (parse-e s)
  (match s
    ...
    [(list 'with-handlers hs e)
     (WithEx (parse-error-handlers hs) (parse-e e))]
    [(cons e es)
     (App (parse-e e) (map parse-e es))]
    [_ (error "Parse error" s)]))

(define (parse-error-handlers hs)
  (match hs
    ['() '()]
    [(cons x xs) 
      (cons (parse-error-handler x) (parse-error-handlers xs))]))

(define (parse-error-handler h)
    (match h
      [(list (? pred1? p) e)  (let ((x (gensym 'handler)))
                                (Handler (App (Prim1 p x) _ )(parse-e e)))]
      [(list (list (? pred2? p) e1) e2)
        (Handler (Apply p (list (parse-e e1))) (parse-e e2))]
      [(list fun e)
       (Handler (Apply fun '()) (parse-e e))]))
...
(define pred1s
  '(zero? char? eof-object? empty? cons? box? vector? string?))
(define pred2s
  '(eq?))

(define (pred1? x)
    (if (eq? (memq x pred1s) #f) #f #t))

(define (pred2? x)
    (if (eq? (memq x pred2s) #f) #f #t))
