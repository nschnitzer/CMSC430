...
;; type WithEx = ((Listof Handler) Expr)
(struct WithEx (hs e)  #:prefab)
...
;;           | (Raise Expr)
;;           | (Handler (Pair Expr Exor))
;;           | (WExceptions (Listof Handler) Expr)
;;           | (Apply Expr Expr)
;; type Id   = Symbol
;; type Op0  = 'read-byte
...
(struct Handler (a f)     #:prefab)
(struct Apply (exp es)       #:prefab)
